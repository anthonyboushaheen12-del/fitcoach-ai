...
/** Create-or-update the active workout plan JSON (e.g. apply AI corrected split). */
export async function POST(request) {
  ...
  try {
    const body = await request.json()
    const { profileId, content, trainerId } = body || {}
    ...
    const { error: insErr } = await supabase.from('plans').insert({
      ...
      type: 'workout',
      content,
      trainer: typeof trainerId === 'string' && trainerId.trim() ? trainerId.trim() : null,
      active: true,
    })
    if (insErr) {
    ...
    console.error('workout-plan POST:', e)
    return Response.json(
      { success: false, error: e.message || 'Failed to update plan' },
      { status: 500 }
    )
